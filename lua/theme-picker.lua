...
-- ── Persistence ─────────────────────────────────────────────
-- Saves both the colorscheme name AND the display label so we can
-- find the right catalog entry (and its setup hook) on reload.

local persist_path = vim.fn.stdpath("data") .. "/last_colorscheme.json"
...
local function save_colorscheme(entry)
    ...
    if f then
        f:write(vim.json.encode({ name = entry[1], label = entry[2] }))
        f:close()
    end
end
...
local function load_saved_colorscheme()
    ...
    if not data then
        -- Try legacy plain-text file
        local legacy = io.open(vim.fn.stdpath("data") .. "/last_colorscheme.txt", "r")
        if legacy then
            local name = legacy:read("*l")
            legacy:close()
