...
local catalog = {
    ...
    },
    {
        ...
        function()
            local k = require("kanagawa")
            k.config.theme = "dragon"
            k.config.background.dark = "dragon"
            vim.o.background = "dark"
        end,
    },
    {
        ...
        function()
            local k = require("kanagawa")
            k.config.theme = "lotus"
            k.config.background.light = "lotus"
...
local function build_lines(filter)
    ...
    local function flush_group()
        if not current_header or #group_items == 0 then
            return
        end
        ...
        if #visible > 0 then
            ...
            for _, vi in ipairs(visible) do
                ...
                local is_active = (cs_name == active_cs) and (not vi[5] or active_label == label) -- no setup_fn → name match is enough
        ...
        if not entry then
            if line ~= "" then
                vim.api.nvim_buf_set_extmark(buf, ns, i - 1, 0, {
                    end_col = #line,
                    hl_group = "ThemePickerHeader",
                })
            end
        else
            local active = vim.g.colors_name or ""
            local is_active_entry = (entry[1] == active) and (not entry[5] or active_label == entry[2])
            if is_active_entry then
                vim.api.nvim_buf_set_extmark(buf, ns, i - 1, 0, {
                    end_col = #line,
                    hl_group = "ThemePickerActive",
                })
            else
...
function M.open()
    ...
    vim.wo[win].winhighlight =
        "NormalFloat:Normal,FloatBorder:ThemePickerBorder,FloatTitle:ThemePickerTitle,FloatFooter:ThemePickerDim,CursorLine:ThemePickerCursorLine,Cursor:ThemePickerHiddenCursor"
    ...
    local function open_search()
        ...
        vim.wo[search_win].winhighlight =
            "NormalFloat:Normal,FloatBorder:ThemePickerSearchBorder,FloatTitle:ThemePickerSearchPrompt"
